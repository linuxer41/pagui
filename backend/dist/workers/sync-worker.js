...
var init_wallet_repository = __esm(() => {
  ...
  walletRepository = {
    ...
    async listAll(filters = {}) {
      ...
      return { wallets: r.rows, totalCount: parseInt(c.rows[0].t) };
    },
    ...
    `, [
        ...
        data.senderDocumentId || null,
        data.senderAccount || null,
        data.senderBankCode || null,
    ...
    async getMovements(walletId, filters = {}) {
      ...
      if (filters.type) {
        pc++;
        conditions.push(`movement_type = $${pc}`);
      ...
      return { movements: r.rows, totalCount: parseInt(c.rows[0].t) };
    },
  ...
  emit(event, ...args) {
    this.listeners.get(event)?.forEach((fn) => {
      try {
        ...
        logger.error("EventBus error", { event, error: String(e) });
      }
    });
  }
  removeAll() {
