...
export const userRepository = {
  async create(data: CreateUserData): Promise<UserRow> {
    const r = await query(`
      INSERT INTO users (id, email, password, full_name, phone, address, role_id, status)
      VALUES ($1, $2, $3, $4, $5, $6, $7, 'active')
      RETURNING id
    `, [data.id, data.email, data.password, data.fullName, data.phone || null, data.address || null, data.roleId])
  ...
  async list(filters: { page?: number; limit?: number; search?: string; status?: string; roleId?: bigint } = {}) {
    ...
    if (filters.roleId) {
      pc++; conditions.push(`u.role_id = $${pc}`); params.push(filters.roleId)
    }
