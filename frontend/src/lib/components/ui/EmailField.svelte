<script lang="ts">
  ...
  let { label = 'Correo', value = $bindable(''), placeholder = 'correo@ejemplo.com', onInput = undefined as ((e: Event) => void) | undefined, disabled = false, error: externalError = '' } = $props()
  let internalError = $state('')
  let inputId = $state(`ef-${Math.random().toString(36).slice(2, 8)}`)
  let error = $derived(externalError || internalError)
  function validate() {
    internalError = value && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value) ? 'Correo inválido' : ''
  }
</script>
...
<div class="field">
  ...
  <div class="field-wrap" class:has-error={error}>
    ...
    <input type="email" bind:value {placeholder} {disabled} oninput={onInput} onblur={validate} class="field-input" id={inputId} />
  </div>
  {#if error}
