...
<style>
  ...
  .balance-card { text-align: center; padding: var(--space-8) var(--space-6); background: linear-gradient(135deg, rgba(var(--primary-rgb), 0.35) 0%, rgba(var(--surface-rgb), 1) 60%); border-radius: var(--radius-2xl); position: relative; overflow: hidden; color: rgba(var(--text-primary-rgb), 1); }
  ...
  .balance-label { font-size: var(--text-xs); font-weight: 500; opacity: 0.7; letter-spacing: 0.5px; text-transform: uppercase; margin-bottom: var(--space-1); position: relative; z-index: 1; }
  .balance-amount { font-size: 2.25rem; font-weight: 800; letter-spacing: var(--tracking-tight); line-height: 1.1; position: relative; z-index: 1; }
  .balance-sub { font-size: var(--text-sm); opacity: 0.6; margin-top: var(--space-2); position: relative; z-index: 1; }
