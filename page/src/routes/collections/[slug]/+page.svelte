<script lang="ts">
  ...
  import FormularioBusqueda from '$lib/components/collections/FormularioBusqueda.svelte';
  import ClienteInfo from '$lib/components/collections/ClienteInfo.svelte';
  import ListaDeudas from '$lib/components/collections/ListaDeudas.svelte';
  import QRDisplay from '$lib/components/collections/QRDisplay.svelte';
  import DebugMessage from '$lib/components/collections/DebugMessage.svelte';
  import EmpsaatDeudasDisplay from '$lib/components/collections/EmpsaatDeudasDisplay.svelte';
  ...
  // Si la empresa no tiene propiedad usaQR, asumimos que s├¡ lo usa
  ...
  let isGeneratingQR = false; // Variable para controlar el estado de carga del bot├│n de pagar
  ...
  // Funci├│n para buscar cuenta usando el patr├│n correcto de SvelteKit
  async function buscarCuenta() {
    if (!codigoClienteInput.trim()) {
      error = 'Por favor ingresa tu c├│digo de cliente';
      return;
    }
    ...
    try {
      ...
      if (actionResult.type === 'success' && actionResult.data) {
        ...
        // La informaci├│n del cliente estar├í en searchResult.data.deudas[0].abonado
        
        // Limpiar el input despu├⌐s de una b├║squeda exitosa
        codigoClienteInput = '';
      ...
      // Aplicar la acci├│n del resultado
  ...
  // Funci├│n para limpiar la informaci├│n del cliente y volver al formulario de b├║squeda
  function limpiarCliente() {
    ...
    // Limpiar polling si est├í activo
    if (pollingInterval) {
  ...
  async function actualizarDeudas() {
    ...
    try {
      ...
      const actionResult: ActionResult = deserialize(await response.text());
      console.log('Actualizaci├│n - result', actionResult);
      
      if (actionResult.type === 'success' && actionResult.data) {
        // Actualizar solo los datos sin resetear el estado
        searchResult = actionResult.data;
        console.log('Actualizaci├│n - searchResult actualizado:', searchResult);
      } else if (actionResult.type === 'failure') {
  ...
  // Funci├│n para generar QR espec├¡fica para EMPSAAT usando el nuevo flujo de transacciones
  async function generarQREmpsaat(deudas: any[], total: number, abonado: any) {
    ...
    try {
      if (empresa.usaQR) {
        ...
        console.log('Enviando datos para crear transacci├│n:', {
        ...
        if (actionResult.type === 'success' && actionResult.data) {
          ...
          if (apiResponse.success) {
            // La respuesta del servidor ya incluye tanto la transacci├│n como el QR
            ...
            error = apiResponse.message || 'Error al crear transacci├│n y generar QR';
          }
        } else if (actionResult.type === 'failure') {
          if (Array.isArray(actionResult.data) && actionResult.data.length >= 4) {
            ...
            error = 'Error al crear transacci├│n y generar QR';
          }
        } else {
          error = 'Error inesperado al crear transacci├│n y generar QR';
        }
      }
      ...
      if (err instanceof Error) {
        // Si el error contiene informaci├│n espec├¡fica de la API, mostrarla
        if (err.message.includes('No se puede crear la transacci├│n')) {
  ...
  // Funci├│n para generar QR o pagar facturas de agua
  async function generarQR(deuda: Deuda) {
    ...
    // Variable para almacenar el resultado de la acci├│n
    let actionResult: ActionResult | undefined;
    ...
    try {
     ...
     if (empresa.usaQR) {
      ...
      // Mejorar descripci├│n si es posible
      ...
      const response = await fetch(`?/generarQR`, {
       ...
       if (actionResult.type === 'success' && actionResult.data) {
         ...
         // Manejar el formato espec├¡fico de error que viene en actionResult.data
         if (Array.isArray(actionResult.data) && actionResult.data.length >= 4) {
       ...
       if (actionResult.type === 'success' && actionResult.data) {
         ...
         alert(`Pago procesado correctamente: ${actionResult.data.mensaje || 'Operaci├│n exitosa'}`);
     ...
     if (actionResult) applyAction(actionResult);
      ...
      // Desactivar loader del bot├│n
      isGeneratingQR = false;
    }
  }
  
  // Funci├│n para verificar estado del QR
  async function verificarEstadoQR(qrId: string) {
    try {
     ...
     const actionResult: ActionResult = deserialize(await response.text());
      ...
      if (actionResult.type === 'success' && actionResult.data?.success) {
        qrStatus = actionResult.data.data;
          ...
          // Si el QR est├í pagado, usado o expirado, detener el polling
     ...
     if (actionResult) applyAction(actionResult);
      ...
      console.error('Error de conexi├│n al verificar estado:', err);
      return null;
    }
  }
  
  // Funci├│n para iniciar el polling del estado
  ...
  async function pagarServicios(servicios: Deuda[], total: number) {
    ...
    try {
      ...
      // Usuario que realiza la operaci├│n (podr├¡a venir de un sistema de autenticaci├│n)
      ...
      if (actionResult.type === 'success' && actionResult.data) {
        // Mostrar resultado exitoso
        alert(`Pago de servicios procesado correctamente: ${actionResult.data.mensaje || 'Operaci├│n exitosa'}`);
  ...
  // Funci├│n para limpiar la b├║squeda y volver al estado inicial
  function limpiarBusqueda() {
    ...
    detenerPollingEstado();
     ...
     // Funci├│n para descargar el QR completo usando html2canvas
   ...
   async function compartirQR(qrImage: string, qrId: string) {
     try {
       if (navigator.share) {
         ...
         alert('Informaci├│n del QR copiada al portapapeles');
       }
     } catch (err) {
   ...
   const takeScreenshot = async () => {
    ...
    const qrElement = document.querySelector('#qr-capture-content') as HTMLElement;
       if (!qrElement) {
         console.error('No se encontr├│ el elemento del QR para descarga');
         return;
       }
  ...
  // Actualizar el paso actual seg├║n el estado del proceso
  ...
  function goToPreviousStep() {
    if (currentStep > 1) {
      ...
      // Limpiar estado seg├║n el paso
      if (currentStep === 1) {
        // Volver al paso 1: limpiar b├║squeda
        searchResult = null;
        cliente = undefined;
        qrGenerado = null;
        qrStatus = null;
  ...
  function goToStep(step: number) {
    if (step < currentStep) {
      ...
      // Limpiar estado seg├║n el paso al que se va
      if (step === 1) {
        // Ir al paso 1: limpiar b├║squeda
        searchResult = null;
        cliente = undefined;
        qrGenerado = null;
        qrStatus = null;
...
<svelte:head>
  ...
  <meta name="description" content="Paga tu cuenta de {empresa.nombre} de forma segura y r├ípida" />
...
<div class="dashboard-layout">
  <!-- Sidebar/Header (se convierte en header en m├│vil) -->
    ...
    <div class="sidebar-content">
      ...
      <div class="sidebar-footer">
        <a href={empresa.webUrl} class="return-link">
          <span class="return-icon">ΓåÉ</span>
  ...
  <main class="main-content">
          ...
          <div class="process-steps">
              ...
              >
                ...
                <div class="step-content">
                  ...
                  <span class="step-description">Ingresa tu n├║mero de cuenta</span>
            </div>
            </div>
    ...
    <!-- Footer m├│vil (parte del scroll del main content) -->
...
<style>
  ...
  /* Informaci├│n de la empresa en sidebar */
  .company-section {
  ...
  /* Estado inactivo - cuando estamos en b├║squeda */
  ...
  /* Secci├│n de b├║squeda m├ís prominente */
  .search-section {
    background: transparent;
    border-radius: 0;
    border: none;
